...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/finaldb"
## Provided:
...
def allops():
	...
	return render_template('allops.html', all=all, form=form)
...
def one_op(ident):
	...
	opinion = Opinion.query.filter_by(opinionid = ident).first()
	return render_template('opiniontpl.html', op = opinion, form = form)
...
# TODO 364: Complete route to delete a whole opinion
@app.route('/delete/<lst>',methods=["GET","POST"])
def delete(lst):
    item = Opinion.query.filter_by(opinionid = int(lst)).first()
    title = item.name
    ...
    return redirect(url_for('allops'))     # And should redirect the user to the page showing all the todo lists
